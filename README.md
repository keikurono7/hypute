...
`bench/gating_benchmark.cpp` (200k ticks, 4096 channels, 1.5% active). On a laptop i5-10300H the gated path runs the same work in roughly **one-sixth to one-eighth the time** of a dense scan (about 400-650 ns/tick vs 3,300-4,000 ns/tick, depending on CPU and load).

Both engines are checksum-verified to produce identical output, and a `volatile` sink prevents the compiler from optimizing the work away, so the number is real, not a dead-code illusion. Absolute ns are hardware- and load-dependent; the benchmark prints the CPU, and the [CI run](https://github.com/keikurono7/hypute/actions/workflows/ci.yml) is the canonical, anyone-can-reproduce figure.

The primitive is header-only and importable, not just a demo: [`include/hypute_gate.h`](include/hypute_gate.h) exposes `hypute_for_each_set_bit(mask, nblocks, fn)` (the exact code this benchmark measures), so you can drop it into your own hot loop.
...
- **Gating** ([`include/hypute_gate.h`](include/hypute_gate.h), measured in `bench/gating_benchmark.cpp`): sparse events packed as bits in 64-bit words; `__builtin_ctzll` + `reg &= reg-1` iterate only set bits; all state cache-resident and allocation-free.
